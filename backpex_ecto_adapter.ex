defmodule Insi.BackpexEctoAdapter do
  @config_schema [
    repo: [
  ...
  defp search_conditions([field | searchable_fields], search_string) do
    dynamic(
      ^search_condition(field, search_string) or
        ^search_conditions(searchable_fields, search_string)
    )
  end
  ...
  defp where_id(query, schema_name, id_field, :id, id) do
    case Ecto.Type.cast(:id, id) do
      {:ok, valid_id} ->
        where(query, [{^schema_name, schema_name}], field(schema_name, ^id_field) == ^valid_id)
  ...
  defp associations(fields, schema) do
    fields
    |> Enum.filter(fn {_name, field_options} = field ->
      field_options.module.association?(field)
    end)
    |> Enum.map(fn
      {name, field_options} ->
  ...
  def get_primary_key_field(%{__struct__: struct}) when is_atom(struct),
    do: get_primary_key_field(struct)
